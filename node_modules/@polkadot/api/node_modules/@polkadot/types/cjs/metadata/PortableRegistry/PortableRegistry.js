...
function extractNameFlat(portable, lookupId, params, path) {
  ...
  for (let i = 0; i <= last; i++) {
    const p = (0, _util.stringPascalCase)(isInternal ? path[i].replace('pallet_', '') : path[i]);
...
function extractName(portable, _ref2) {
  let {
    id,
    type: {
      params,
...
function removeDuplicateNames(lookup, portable, names) {
  const rewrite = {};
  return names.map(_ref3 => {
    let [lookupIndex, name, params] = _ref3;
    ...
    const allSame = names.filter(_ref4 => {
      let [, oName] = _ref4;
      return name === oName;
    }); // are there among matching names

    const anyDiff = allSame.some(_ref5 => {
      let [oIndex,, oParams] = _ref5;
    ...
    const paramIdx = allSame[0][2].findIndex((_ref6, index) => {
      let {
        type
      } = _ref6;
      return allSame.every(_ref7 => {
        let [,, params] = _ref7;
        return params[index].type.isSome;
      }) && allSame.every((_ref8, aIndex) => {
        let [,, params] = _ref8;
    ...
    for (let i = 0; i < allSame.length; i++) {
      ...
      const {
        def,
        path
      } = lookup.getSiType(oParams[paramIdx].type.unwrap()); // if it is not a primitive and it doesn't have a path, we really cannot
    ...
    for (let i = 0; i < allSame.length; i++) {
      ...
      adjusted[i] = [oIndex, `${oName}${flat[1]}`];
    } // check to see if the adjusted names have no issues
    ...
    return null;
  }).filter(n => !!n).map(_ref9 => {
    let [lookupIndex, name, params] = _ref9;
