(function (global, factory) {
  ...
  function pairOfReplacer(v) {
    return `(${v},${v})`;
  }
  function removePairOf() {
    return value => replaceTagWith(value, 'PairOf<', pairOfReplacer);
  }
  ...
  function wrapReplacer(v) {
    return v;
  }
  function removeWrap(check) {
    return value => replaceTagWith(value, check, wrapReplacer);
  }
  ...
  function extractNameFlat(portable, lookupId, params, path, isInternal = false) {
    ...
    for (let i = 0; i <= last; i++) {
      const p = util.stringPascalCase(isInternal ? path[i].replace('pallet_', '') : path[i]);
